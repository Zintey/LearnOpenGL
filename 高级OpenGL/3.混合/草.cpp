...
int main()
{
	...
	Shader grassShader("shader/vertex_common_shader.glsl", "shader/fragment_transparent_shader.glsl");
	//Shader grassShader("shader/vertex_common_shader.glsl", "shader/fragment_common_shader.glsl");
	...
	unsigned int grassTexture = TextureFromFile("grass.png", "texture");
	...
	std::vector<float> vegetationRotations = {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < vegetationPositions.size(); i++)
		{
			...
			grassModel = glm::translate(grassModel, vegetationPositions[i] + glm::vec3(0.0f, 0.0f, 0.0f));

			grassModel = glm::rotate(grassModel, glm::radians(vegetationRotations[i]), glm::vec3(0.0f, 1.0f, 0.0f));
			grassModel = glm::rotate(grassModel, glm::radians(90.f), glm::vec3(1.0f, 0.0f, 0.0f));
			grassModel = glm::scale(grassModel, glm::vec3(0.5f));

			grassShader.setMatrix4("model", grassModel);
			glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		}
