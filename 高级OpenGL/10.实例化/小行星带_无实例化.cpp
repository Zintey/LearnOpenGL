...
int main()
{
	...
	Shader shader("shader/vertex_小行星带_planet.glsl", "shader/fragment_小行星带_planet.glsl");

	unsigned rockAmount = 20000;
	glm::mat4* rockModels = new glm::mat4[rockAmount];
	srand(glfwGetTime());
	float radius = 50.0;
	float offset = 3.5f;
	for (int i = 0; i < rockAmount; i++)
	{
		...
		model = glm::rotate(model, rotateAngle, glm::vec3(0.3f, 0.4f, 0.2f));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < rockAmount; i++)
		{
			shader.setMatrix4("model", rockModels[i]);
